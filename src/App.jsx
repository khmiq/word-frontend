...
const App = () => {
  ...
  const handleAddWord = async () => {
    const trimmedWord = newWord.trim().toLowerCase();  // Normalize to lowercase and trim spaces
    if (!trimmedWord) return;

    const existingWord = words.find((word) => word.text.toLowerCase() === trimmedWord);  // Compare case-insensitively
    if (existingWord) {
      toast.error("This word already exists");
    ...
    try {
      const response = await axios.post(`${API_URL}/words`, { text: trimmedWord }); // Send normalized word to backend
  ...
  const handleEditWord = async () => {
    ...
    const updatedWord = editingWord.text.trim().toLowerCase();  // Normalize the edited word to lowercase

    try {
      await axios.patch(`${API_URL}/words/${editingWord._id}`, { text: updatedWord });

      setWords(words.map((word) => (word._id === editingWord._id ? { ...word, text: updatedWord } : word)));
